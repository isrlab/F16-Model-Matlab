...
% Control surface bounds -- throttle, elevator, aileron, rudder
control.lb = [1000*lbf2N -25*d2r -21.5*d2r -30*d2r];
control.ub = [19000*lbf2N 25*d2r  21.5*d2r  30*d2r];
control.guess = [10000*lbf2N,0,0,0]; % Initial guess for trim control
...
%
% This is the ordering: [U,w,th,T,dele], with units m/s, m/s, rad, N, rad
lb = horzcat(Vtmin,Vtmax*tan(alpha.min),alpha.min,control.lb(1:2));
ub = horzcat(Vtmax,Vtmax*tan(alpha.max),alpha.max,control.ub(1:2));
p0 = [Vt0,0,0,control.lb(1),0];
t0 = 0;
...
F16([],[],[],'term');
    ...
    function J = trimCost(optParam)
        ...
        y0 = F16(t0, x, u, 'outputs'); % Need this to update the dynamics with current state values
        xdot = F16(t0,x,u,'derivs');
    ...
    function [x,u] = xu(optParam) % Specific to steady-level flight
        ...
        u = zeros(1,7); % total input = 4 controls + 3 gusts 
        u(1) = T; % Newtons
        u(2) = dele; % radians
    end
